...
class AddProductDiscount extends StatefulWidget {
  AddProductDiscount({Key key,this.discountId,this.storeId,this.token}) : super(key: key);
...
class _AddToopingsState extends State<AddProductDiscount> {
  var deliveryBoyId;


  List<Products> additionals = [],listWithDiscount=[];
  ...
  void initState() {
    ...
    Utils.check_connectivity().then((value) {
      if (value) {
        ...
        pd.show();
        networksOperation.getProductsWithoutDiscount(context, widget.token,widget.storeId).then((value) {
          pd.hide();
          setState(() {
            ...
            // }
            additionals = value;
            print(additionals.toString() + "DisCounted Product List");
