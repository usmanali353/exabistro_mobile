...
class AddStockDetails extends StatefulWidget {
  var token,storeId,purchaseOrderId,purchaseItemId;
 int stockItemId;
  AddStockDetails({this.storeId,this.stockItemId,this.token,this.purchaseOrderId,this.purchaseItemId}); // var product_id;
  @override
...
class _add_SizesState extends State<AddStockDetails> {
  String token;
 ...
 // Vendors selectedVendor;
  ...
  void initState(){
    ...
    //   }
    // });
    networksOperation.getVendorsByStockId(context, widget.token,widget.stockItemId).then((value) {
      setState(() {
        if(vendorsList!=null)
          vendorsList.clear();
        vendorsList = value;
                  ...
                  Padding(
                    ...
                    child: TextFormField(
                      ...
                      decoration: InputDecoration(
                        ...
                        labelText: "Actual Purchase Quantity",
                        labelStyle: TextStyle(color:yellowColor, fontWeight: FontWeight.bold),
                  ...
                  InkWell(
                    onTap: (){
                      if (!_formKey.currentState.validate()) {
                        ...
                        var data= StockItems.addStockItemsDetailToJson(StockItems(
                          storeId: widget.storeId,
                           id: widget.stockItemId,
                          ...
                          purchasedQuantity: double.parse(purchaseQuantity.text),
                          purchaseOrderItemId: widget.purchaseItemId,
                          expiryDate: expireDate
                          //totalStockQty: double.parse(quantity.text)
                        ));
                        Utils.check_connectivity().then((result){
                          if(result){
                            networksOperation.addStockItemDetail(context, token,data).then((value) {
                              if(value){
