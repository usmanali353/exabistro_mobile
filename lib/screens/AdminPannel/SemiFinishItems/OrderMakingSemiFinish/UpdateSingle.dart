...
class UpdateOrderMakingSingle extends StatefulWidget {
  var storeId;
  SemiFinishItems semiFinishObj;
  SemiFinishedDetail detailObj;

  UpdateOrderMakingSingle(this.storeId,this.semiFinishObj,this.detailObj);
...
class _add_CategoryState extends State<UpdateOrderMakingSingle> {
  String token;
  File _image;
  var picked_image;
  var responseJson;
  ...
  void initState(){
    ...
    SharedPreferences.getInstance().then((value) {
      setState(() {
       ...
       quantity.text = widget.detailObj.quantity;
       expireDate = widget.detailObj.expiryDate;
      });
    });
                      ...
                      Utils.check_connectivity().then((result){
                        if(result){
                          networksOperation.addSemiFinishDetails(context, token, SemiFinishedDetail(
                              quantity: int.parse(quantity.text),semiFinishedItemId: widget.semiFinishObj.id,
