import 'package:capsianfood/ReviewApp/Category/Category.dart';
...
class FeedBackBusinessList extends StatefulWidget {
  var restaurant,roleId;


  FeedBackBusinessList(this.restaurant,this.roleId);
...
class _NewStoresState extends State<FeedBackBusinessList> {
  String token;
        ...
        onRefresh: (){
          return Utils.check_connectivity().then((result){
            if(result){
              ...
              ReviewNetworks.getBusinessByParentId(context,widget.restaurant['id']).then((value){
                setState(() {
                  print(value);
                  businessList = value;
                              ...
                              child: Column(
                                children: [
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          ...
                                          Text("${businessList[index].address}", style: TextStyle(color: PrimaryColor, fontSize: 15, fontWeight: FontWeight.bold),)
                                        ],
                                      ),
                                    ],
                                  ),
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      Text("FeedBack",style: TextStyle(fontSize: 18,color: PrimaryColor,fontWeight: FontWeight.bold),),
                                      InkWell(
                                        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context)=> IndividualFeedbacks(businessId: businessList[index].id,))),
                                        child: Padding(
                                          padding: const EdgeInsets.all(10),
                                          child: FaIcon(Icons.announcement, color: PrimaryColor, size: 35,),
                                        ),
                                      ),
                                      SizedBox(width: 15,),
                                      Text("Categories",style: TextStyle(fontSize: 18,color: PrimaryColor,fontWeight: FontWeight.bold),),
                                      InkWell(
                                        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context)=> ReviewCategoryList( businessList[index].id,))),
                                        child: Padding(
                                          padding: const EdgeInsets.all(10),
                                          child: FaIcon(FontAwesomeIcons.list, color: PrimaryColor, size: 35,),
                                        ),
                                      ),
                                    ],
                                  )
